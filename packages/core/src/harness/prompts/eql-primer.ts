...
export const EQL_PATCH_OPERATIONS = [
  ...
  "- Request says remove all / clear / start fresh / wipe steps → DELETE STEP once for every existing step id (keep PATCH WORKFLOW id).",
  "- Request says start fresh with new steps → DELETE every existing step id, then ADD STEP for the new graph (no AFTER/BEFORE when the workflow is empty).",
  ...
  "- CLEAR ACCEPTS / CLEAR RETURNS only clear I/O schemas — they do not remove steps.",
  "",
  ...
  "- Empty workflows are valid: DELETE every listed step id when the user wants no steps left.",
].join("\n")
...
export const EQL_PATCH_CANONICAL_EXAMPLES = [
  ...
  "DELETE STEP actions",
  "",
  "Example 7 — remove all steps (clear / start fresh):",
  "User: Remove all steps from the workflow.",
  "Output:",
  "PATCH WORKFLOW haiku-explain",
  "DELETE STEP haiku",
  "DELETE STEP explain",
  "",
  "Example 8 — start fresh with one new step:",
  "User: Clear the workflow and start fresh with one @executioncontrolprotocol/chrome-ai.generate step that writes a haiku.",
  "Output:",
  "PATCH WORKFLOW haiku-explain",
  "DELETE STEP haiku",
  "DELETE STEP explain",
  "ADD STEP haiku USES @executioncontrolprotocol/chrome-ai.generate",
  "  LABEL \"Write Haiku\"",
  "  WITH prompt = \"Write a haiku.\"",
  "  AS haiku",
  "",
  "Example 9 — MOVE STEP reorder:",
